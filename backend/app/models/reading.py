...
class Reading(db.Model):
    ...
    metadata = db.Column(db.JSON)  # battery_level, rssi, temperature, etc.
    ...
    def create_from_mqtt(cls, mqtt_data):
        ...
        reading = cls(
            ...
            metadata=mqtt_data.get('metadata', {}),
        ...
        if reading.metadata:
            if 'battery_level' in reading.metadata:
                sensor.battery_level = reading.metadata['battery_level']
            if 'rssi_dbm' in reading.metadata:
                sensor.signal_strength = reading.metadata['rssi_dbm']
